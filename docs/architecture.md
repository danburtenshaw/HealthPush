...
├──────────┬──────────┬───────────────────┤
│ Amazon   │ Home     │ Future            │ Concrete
│ S3       │ Assistant│ Destinations      │ Implementations
└──────────┴──────────┴───────────────────┘
```
...
- **`S3Destination`** -- Direct export to an S3 bucket with JSON or CSV output and deduplicated uploads.
- **`HomeAssistantDestination`** -- Webhook-based sync to a Home Assistant instance via the custom integration.
